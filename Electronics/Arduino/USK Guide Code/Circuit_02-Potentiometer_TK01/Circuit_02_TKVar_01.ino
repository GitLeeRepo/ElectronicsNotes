...
Example sketch 02 TK Variation 01

TK: Modified the circuit to dim/brighten the LED, rather
then alter its blink time.  In so doing I removed both the
A0 pin and the A13 pin, so in effect this script no longer 
has any effect on the bread board circuit, although the
LED on the UNO is still effected (it blinks, but is unaffected
by the potentiometer changes).  In place of the A0 connecting
to the middle lead of the potentiometer(via E7) and A13 connecting
to the positive side of the LED (via J20), I instead linked
the middle lead of the potentiometer (via E7) directly to the 
positive side of the LED (via J20).  The only thing the UNO
does now is provide power and ground through the 5V and GND
pins to the bread boards positive and negative strips
respectively.
